/**
 * Клиентский поиск для справки
 * Версия 3.1 - MiniSearch встроен + HelpSearch
 */

// ============================================================================
// MiniSearch 7.1.2 (встроенная версия)
...
// ============================================================================
// HelpSearch - Клиентский поиск для справки
// ============================================================================
const HelpSearch = {
  miniSearch: null,
  tocTree: null,
  ...
  stopWords: new Set([
    ...
    'только', 'её', 'ее', 'мне', 'было', 'вот', 'от', 'меня', 'еще', 'ещё', 'нет',
    'о', 'об', 'из', 'ему', 'теперь', 'когда', 'уже', 'вам', 'ни', 'быть', 'был',
    ...
    'might', 'must', 'shall', 'can', 'need', 'it', 'this', 'that', 'these', 'those'
  ]),

  /**
   * Русский стемминг - упрощённая версия
   */
  stem(word) {
    if (word.length < 4) return word;

    // Окончания для удаления (от длинных к коротким)
    const suffixes = [
      // Глаголы
      ...
      'айший', 'айшая', 'айшее', 'айшие',
      // Существительные
      'ность', 'тель', 'ение', 'ание', 'ство',
      'ости', 'ость',
      // Падежные окончания
      'ами', 'ями', 'ому', 'ему', 'ого', 'его',
      'ыми', 'ими', 'ах', 'ях', 'ов', 'ев', 'ей',
  ...
  /**
   * Обработка терма для индексации и поиска
   */
  processTerm(term) {
    if (!term || typeof term !== 'string') return null;
    const lower = term.toLowerCase();
    if (lower.length < 2) return null;
    if (this.stopWords.has(lower)) return null;
    return this.stem(lower);
  },

  /**
   * Инициализация поиска
   * Данные загружаются через <script src="search_index.js"> (window.__SEARCH_DATA__)
   * Это работает и по file://, и по http:// без проблем с CORS
   */
  async init() {
    ...
    try {
      // Ждём пока search_index.js загрузится и установит window.__SEARCH_DATA__
      let attempts = 0;
      while (!window.__SEARCH_DATA__ && attempts < 50) {
        await new Promise(r => setTimeout(r, 100));
        attempts++;
      }

      const json = window.__SEARCH_DATA__;
      if (!json) {
        console.warn('Search index not found (window.__SEARCH_DATA__ is empty)');
        return;
      }

      // Проверяем формат индекса
      if (json.miniSearchIndex) {
        // Формат с сериализованным MiniSearch индексом
        await this._loadMiniSearchIndex(json);
      } else if (json.entries) {
        // Формат с записями — создаём индекс на лету
        await this._buildIndexFromEntries(json);
      } else {
        console.warn('Unknown search index format');
        return;
      }
  ...
  /**
   * Загрузка предсозданного MiniSearch индекса
   */
  async _loadMiniSearchIndex(json) {
    const options = {
      fields: ['title', 'content'],
      storeFields: ['title', 'url', 'content'],
      processTerm: (term) => this.processTerm(term),
      tokenize: (text) => text.split(/[\s\-_.,;:!?()\[\]{}'"«»<>\/\\]+/).filter(t => t.length >= 2),
      searchOptions: {
        boost: { title: 3 },
        fuzzy: 0.2,
        prefix: true
    ...
    // Загружаем асинхронно чтобы не блокировать UI
    if (typeof MiniSearch.loadJSONAsync === 'function') {
      this.miniSearch = await MiniSearch.loadJSONAsync(json.miniSearchIndex, options);
    } else {
      this.miniSearch = MiniSearch.loadJSON(json.miniSearchIndex, options);
    }

    // Загружаем TOC для breadcrumbs
    this.tocTree = json.toc || null;
    if (this.tocTree) {
      this._buildBreadcrumbMap(this.tocTree.elements, []);
    }

    // Карта URL -> title
    if (json.titles) {
      this.urlToTitle = json.titles;
    }
  },

  /**
   * Создание индекса из старого формата (fallback)
   */
  async _buildIndexFromEntries(json) {
    ...
    // Добавляем документы с уникальными ID
    const documents = json.entries.map((entry, index) => ({
      id: index,
      title: entry.title || '',
      url: entry.url || '',
  ...
  /**
   * Строит карту URL -> breadcrumb рекурсивно
   */
  _buildBreadcrumbMap(elements, path) {
    if (!elements) return;

    for (const id in elements) {
      const node = elements[id];
      const currentPath = [...path, { text: node.text, url: node.url }];

      if (node.url) {
        const normalizedUrl = node.url.toLowerCase().split('#')[0].split('?')[0];
        this.urlToBreadcrumb[normalizedUrl] = currentPath;
        this.urlToTitle[normalizedUrl] = node.text;
      }

      if (node.child && node.child.elements) {
        this._buildBreadcrumbMap(node.child.elements, currentPath);
      }
    }
  },

  /**
   * Получить breadcrumb для URL
   */
  getBreadcrumb(url) {
    const normalizedUrl = url.toLowerCase().split('#')[0].split('?')[0];
    return this.urlToBreadcrumb[normalizedUrl] || [];
  },

  /**
   * Форматирует breadcrumb в HTML
   */
  formatBreadcrumb(url) {
    const crumbs = this.getBreadcrumb(url);
    if (crumbs.length <= 1) return '';

    const parents = crumbs.slice(0, -1);
    return parents.map(c =>
      '<a href="' + c.url + '" target="hmcontent">' + this._escapeHtml(c.text) + '</a>'
  ...
  search(query, options = {}) {
    if (!query || !this.isReady || !this.miniSearch) return [];
    ...
    try {
      ...
      return results.slice(0, limit).map(result => ({
        url: result.url,
        title: result.title || this.urlToTitle[result.url.toLowerCase()] || result.url,
  ...
  /**
   * Поиск с сниппетами и подсветкой
   */
  searchWithSnippets(query) {
    ...
    return results.map(result => ({
      url: result.url,
      title: result.title,
      titleHighlighted: this._highlightText(result.title, query),
      snippet: this._createSnippet(result.content, query),
      breadcrumb: this.formatBreadcrumb(result.url),
      score: result.score
    }));
  },
   ...
   */
  autoSuggest(query) {
    if (!query || query.length < 2 || !this.isReady || !this.miniSearch) return [];
  ...
  /**
   * Создаёт сниппет с контекстом и подсветкой
   */
  _createSnippet(content, query, maxLength = 200) {
    if (!content) return '';
    ...
    for (const word of words) {
      ...
      if (pos === -1 && stemmed !== word) {
        pos = contentLower.indexOf(stemmed);
      }
    ...
    return this._highlightText(snippet, query);
  },
  ...
  _highlightText(text, query) {
    ...
    for (const word of words) {
      ...
      // Подсвечиваем и оригинал и стем
      const patterns = [escaped];
      if (stemmed !== word) {
        patterns.push(stemmed.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
      }

      for (const pattern of patterns) {
        const regex = new RegExp('(' + pattern + '[а-яёa-z]*)', 'gi');
